...
def EncoderLayer_test(target):
    ...
    assert np.allclose(encoded.numpy(), [[0.5167701, -0.92981905, -0.9731106, 1.3861597],
                                         ...
                                         [0.28154755, -0.3661362, -1.3330412,
                                          1.4176297]]), "Wrong values when training=False"
    print("\033[92mAll tests passed")
...
def Encoder_test(target):
    ...
    encoderq_output = encoderq(x=x, training=True, mask=None)

    assert tf.is_tensor(encoderq_output), "Wrong type. Output must be a tensor"
    assert tuple(tf.shape(encoderq_output).numpy()) == (
        x.shape[0], x.shape[1],
        embedding_dim), f"Wrong shape. We expected ({x.shape[0]}, {x.shape[1]}, {embedding_dim})"
...
def DecoderLayer_test(target, create_look_ahead_mask):
    ...
    padding_mask = None
    out, attn_w_b1, attn_w_b2 = decoderLayerq(x=q, enc_output=encoderq_output, training=True,
    ...
    assert np.allclose(attn_w_b1[0, 0, 1], [0.5271505, 0.47284946, 0.],
                       atol=1e-2), "Wrong values in attn_w_b1. Check the call to self.mha1"
    assert np.allclose(attn_w_b2[0, 0, 1],
                       [0.32048798, 0.390301, 0.28921106]), "Wrong values in attn_w_b2. Check the call to self.mha2"
    ...
    assert np.allclose(out[0, 0], [0.14950314, -1.6444231, 1.0268553,
                                   0.4680646]), "Wrong values in out when we mask the last word. Are you passing the padding_mask to the inner functions?"
...
def Decoder_test(target, create_look_ahead_mask, create_padding_mask):
    ...
    x, attention_weights = decoderk(x=x_array, enc_output=encoderq_output, training=False,
    ...
    assert tf.is_tensor(
        attention_weights[keys[0]]), f"Wrong type for attention_weights[{keys[0]}]. Output must be a tensor"
    ...
    assert np.allclose(attention_weights[keys[0]][0, 0, 1],
                       [0.52145624, 0.47854376, 0.]), f"Wrong values in attention_weights[{keys[0]}]"

    x, attention_weights = decoderk(x_array, encoderq_output, True, look_ahead_mask, None)
    assert np.allclose(x[1, 1],
                       [-0.30814743, -0.6213016, -0.77767026, 1.7071193]), "Wrong values in x when training=True"

    x, attention_weights = decoderk(x_array, encoderq_output, True, look_ahead_mask, create_padding_mask(x_array))
    assert np.allclose(x[1, 1], [-0.0250004, 0.50791883, -1.5877104,
                                 1.1047921]), "Wrong values in x when training=True and use padding mask"
...
def Transformer_test(target, create_look_ahead_mask, create_padding_mask):
    ...
    assert np.allclose(weights[keys[0]][0, 0, 1],
                       [0.4805548, 0.51944524, 0.0, 0.0, 0.0]), f"Wrong values in weights[{keys[0]}]"
